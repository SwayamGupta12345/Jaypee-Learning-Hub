...
<body>
    ...
    <div class="ap_container">
        ...
        <form method="POST" id="deleteForm" class="form" onsubmit="return confirm('Are you sure you want to delete this announcement?');">
            ...
            <select class="sdata select-op" name="data_item" id="data_item" required><!-- sdata means select tag class -->
                <option value="" class = "select-op">Select a news item</option>
                <?php foreach ($newsItems as $newsData): ?>
                    <option class="sdata1 select-op" value="<?php echo htmlspecialchars($newsData); ?>"><?php echo htmlspecialchars($newsData); ?></option>
                <?php endforeach; ?>
            </select>
