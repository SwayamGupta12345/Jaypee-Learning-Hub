...
<body>
    ...
    <div class="ap_container">
        ...
        <form method="POST" onsubmit="return confirmSubmission()">
            ...
            <select id="link_sem" class = "select-op" name="link_sem" onchange="loadSubjects()" required>
                <option class = "select-op" value="">Select Semester</option>
                <?php for ($i = 1; $i <= 10; $i++): ?>
                    <option class = "select-op" value="<?= $i ?>" <?= ($i == $selectedSem) ? 'selected' : '' ?>><?= $i ?></option>
                <?php endfor; ?>
            </select>

            <label for="subject"><b>Subject:</b></label>
            <select id="subject" class = "select-op" name="subject" required>
                <option value="" class = "select-op">Select Subject</option>
                <?php if (!empty($selectedSubjects)): ?>
                    <?php foreach ($selectedSubjects as $subject): ?>
                        <option class = "select-op" value="<?= $subject ?>" <?= ($subject == $selectedSubject) ? 'selected' : '' ?>><?= $subject ?></option>
                    <?php endforeach; ?>
                <?php endif; ?>
            </select>

            <label for="link_description"><b>Description:</b></label>
            <input type="text" id="link_description" class = "select-op" name="link_description" required>

            <label for="link"><b>Link:</b></label>
            <input type="url" id="link" class = "select-op" name="link" required>

            <label for="type"><b>Type:</b></label>
            <select id="type" class = "select-op" name="type" required>
                <option value="" class = "select-op">Select Type</option>
                <option value="college" class = "select-op">College</option>
                <option value="pyq" class = "select-op">Previous Year Question</option>
                <option value="youtube" class = "select-op">YouTube</option>
                <option value="books" class = "select-op">Books</option>
